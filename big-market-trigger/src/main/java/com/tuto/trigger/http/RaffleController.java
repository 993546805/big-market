...
public class RaffleController implements IRaffleService {
    ...
    public Response<List<RaffleAwardListResponseDTO>> queryRaffleAwardList(RaffleAwardListRequestDTO req) {
        try {
            log.info("查询抽奖奖品列表配置开始  strategyId: {}", req.getStrategyId());
            // 查询奖品配置
            List<StrategyAwardEntity> strategyAwardEntities = raffleAward.queryRaffleStrategyAwardList(req.getStrategyId());
            ...
            Response<List<RaffleAwardListResponseDTO>> response = Response.<List<RaffleAwardListResponseDTO>>builder()
                    ...
                    .build();
            log.info("查询抽奖奖品列表配置完毕  strategyId: {}  response: {}", req.getStrategyId(), JSON.toJSONString(response));
            // 返回结果
            return response;
        } catch (Exception e) {
            log.error("查询抽奖奖品列表配置失败  strategyId: {}", req.getStrategyId(), e);
